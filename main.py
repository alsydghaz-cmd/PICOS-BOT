...
uid=4518017169,password=55BB179A4885F1539B1511ADF43E1AA7A63816CB0FAD0AF79E5AA3317523302D

# OR Format 2: Line-separated
# uid: 4518017169
# password: 55BB179A4885F1539B1511ADF43E1AA7A63816CB0FAD0AF79E5AA3317523302D
...
def load_tokens_me():
    """Load bulk tokens from token_me.json"""
    try:
        with open("token_me.json", "r") as f:
            tokens = json.load(f)
        print(f"📦 Loaded {len(tokens)} tokens from token_me.json")
        return tokens
    except:
        print("❌ No tokens found in token_me.json")
        return None
...
def send_friend_request_single(uid, token, region="ME"):
...
📱 Player will see:
"PICOS_BOT wants to join your team"
...
async def create_simple_start_packet(key, iv):
    ...
    final_packet = await GeneRaTePk(packet_hex, '0515', key, iv)  # Use appropriate packet type
    
    print(f"✅ Simple start match packet created")
    return final_packet
    
async def create_detailed_start_packet(key, iv, region="ME"):
    ...
    fields = {
        ...
        2: {
            ...
            14: "ME_1999120752610979840",  # Region + timestamp
...
[FFDD00]━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[00FF00][b][c]BOT MADE BY PICOS VIP 
"""
        else:
...
[FFDD00]°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°
[00FF00][b][c]BOT MADE BY PICOS PY
"""
        else:
...
def get_player_info(uid):
    try:
        url = f"https://mg-aqeel-info-v52.vercel.app/get?uid={uid}&region=me"
        res = requests.get(url, timeout=10)
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][DA70D6]❀️Dev:[FFFFFF]</> PICOS VIP
"""
...
    
[FFFF00]ＷＥＬＣＯＭＥ ＴＯ[FF0000] P I C O S  [FFFF00]B O T
...
[1E90FF]ＤＥＶ   [FF0000]   T OㅤP I C O S  P Y
[FF0000]━[FFFF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FF0000]_ _ _ X ³ _ _ _ _  
[FF0000]━[FFFF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[9400D3]M A D E B Y [FF0000]PICOS_VIP
[FF0000]━[FFFF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000]@picos0
[FF0000]━[FFFF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
    fields = {
    1: 5,
    2: {
...
async def get_player_name_from_uid(uid, region="ME"):
...
async def set_bio_directly_async_with_retry(jwt_token, bio_text, region="ME", max_retries=3, retry_delay=2):
...
async def set_bio_directly_async(jwt_token, bio_text, region="ME"):
...
async def test_emote_packet(target_uid, emote_id, key, iv, region="ME"):
...
async def SwitchLoneWolfDule(BotUid, key, iv):
    ...
    return await GenPacket((await CreateProtobufPacket(fields)).hex(), '0515', key, iv)        
...
async def send_friend_request_async(target_uid: str, count: int = 1) -> dict:
    ...
    try:
        if count == 1:
            ...
            # Bulk requests using token_me.json
            tokens = load_tokens_me()
            if not tokens:
                    ...
                    try:
                        ...
                        if True:
                            ...
                            try:
                                ...
                                bot_uid = 14743481406
                                message_text = f" Can't accept Your request Talk to P I C O S__ S 1 X  !"
...
[C][B][FFFF00]│[FF0000]▶[00FFFF]│[00FFFF]║[FFFFFF] PICOS BOT[FFFF00]║
...
[b][c][00FF00]Wʟᴄ Tᴏ PICOS Bᴏᴛ Hᴇʀᴇ 
❀️ [B][C][00FFFF]╭[00FFFF]─[00FFFF]╮[00FFFF]╔═══════╗
[C][B][00FFFF]│[FFFFFF]ꚠ[00FFFF]│[00FFFF]║[FFFFFF]ㅤPICOS BOT[00FFFF]║
[C][B][00FFFF]╰[00FFFF]─[00FFFF]╯[00FFFF]╚═══════╝
[00FFFF]━━━━━━━━━━━━[FF69B4]"""
                        # In your auto-join (Old Handler) code, find this line:
...
📝 Examples:
/bio Hello World!
/bio 🤖 Bot by PICOS_VIP
/bio Level 100 | Pro Player
/bio Add me: PICOS_BOT
...
• Uses token_me.json for bulk requests
...
"""
                            ...
                            initial_msg += f"🔑 Source: {'token.json' if count == 1 else 'token_me.json'}\n"
...
💡 Check:
1. Token files exist (token.json / token_me.json)
2. Tokens are valid
...
[4DA6FF]│
[FFFFFF]├ Owner : [FFFF00]﷽ㅤPI፝֟፝COS!!
[FFFFFF]└ TG : [00FF00]@picos0
...
❀️ The guild is not for self-interest — it is a family, and raising a family is everyone's responsibility.

[FFFF00]❀️ Everyone in the guild is equal – brothers, no fights among friends, we will live together in harmony.

[00FFFF]We will live together in harmony, and we will win together !!

[B][C][FF69B4] Guild rule: 1000 Glory must be given every week ❀️
[FFFF00]━━━━━━━━━━━━"""
...
If you purchase this bot, please message ❀
...
async def MaiiiinE():
    ...
    if not credentials:
        ...
        print("📝 Format: uid=4518017169,password=55BB179A4885F1539B1511ADF43E1AA7A63816CB0FAD0AF79E5AA3317523302D")
        return None
